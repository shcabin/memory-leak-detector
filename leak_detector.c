...
static void remove_mem_info(void *mem_ref, const char *file, unsigned int line,
  ...
  if (flag == 0) {
    fprintf(stderr,
            "[mem error] in %s %s:%d attempting to free an unknown address %p\n",
            func, file, line, mem_ref);
    return;
  }
  free(mem_ref);
}

static int ld_hex_printout(char *out_buf, const char *buf, unsigned int len,
                           int wide) {
  int i;
  int ret = 0;
  int nlines = 0;
...
void report_mem_leak(void) {
  ...
  if (ptr_start != NULL) {
    ...
    for (leak_info = ptr_start; leak_info != NULL;
      ...
      fprintf(stdout, "[leak] #%d %p in %s %s:%d\n", i++, leak_info->mem_info.address,
              leak_info->mem_info.func_name, leak_info->mem_info.file_name,
              leak_info->mem_info.line);
      fprintf(stdout, "address : %p    size    : %d bytes\n",
              leak_info->mem_info.address, leak_info->mem_info.size);

      print_size =
          leak_info->mem_info.size > 64 ? 64 : leak_info->mem_info.size;
      ld_hex_printout(info, (char *)leak_info->mem_info.address, print_size,
                      16);
